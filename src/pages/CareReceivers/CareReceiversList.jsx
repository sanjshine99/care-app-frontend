...
function CareReceiversList() {
  ...
  const loadCareReceivers = async () => {
    try {
      setLoading(true);
      const response = await careReceiverService.getAll({
        limit: 50,
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {careReceivers.map((cr) => (
                  <tr key={cr._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {cr.dailyVisits?.length || 0} visits
                      </div>
                      {cr.dailyVisits?.some((v) => v.doubleHanded) && (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <button
                        onClick={() => navigate(`/carereceivers/${cr._id}`)}
                      ...
                      <button
                        onClick={() =>
                          navigate(`/carereceivers/${cr._id}/edit`)
                        }
                      ...
                      <button
                        onClick={() => handleDelete(cr._id, cr.name)}
